 ...
 * 让项目文件直观地反映当前工作项目。
 */
function updateMcpJson(mcpPath, project) {
    let config = {};
    try {
...
// ── 配置持久化 ────────────────────────────────────────────────────────────────
function ensurePlaudDir() {
    if (!existsSync(PLAUD_DIR))
        mkdirSync(PLAUD_DIR, { recursive: true });
}
function loadConfig() {
    try {
...
// ── OAuth 授权流程 ────────────────────────────────────────────────────────────
async function doOAuthFlow(apiUrl) {
...
<body><div class="card">
  ...
  <p>${isOk ? 'You can close this window and return to the terminal.' : `Error: ${error ?? 'unknown'}`}</p>
    ...
    await new Promise((resolve, reject) => server.listen(CALLBACK_PORT, '127.0.0.1', () => resolve()).on('error', reject));
    ...
    const code = await Promise.race([
        codePromise,
        new Promise((_, reject) => setTimeout(() => reject(new Error('等待授权超时（10 分钟）')), 600_000)),
    ]).finally(() => {
 ...
 */
function updateClaudeSettings(jwt, project, apiUrl) {
    let settings = {};
...
MCP 配置:   ~/.claude/settings.json
    ...
    if (!force && existingToken) {
        ...
        if (force && existingToken)
            ...
            console.log('Token 不存在或已过期，开始授权流程...');
    ...
    if (mcpPath)
        console.log(`  项目名  ${mcpPath}  (X-Project: ${project})`);
